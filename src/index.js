...
class Site extends EventEmitter {
  constructor() {
    ...
    if (!window.disableSiteCreditsBanner) {
      const size = 40
      const style = `
        font-size: 1px;
        line-height:${size * 0.5}px;padding:${size * 0.25}px ${size * 0.5}px;
        background-size: ${size}px ${size * 0.75}px;
  ...
  initLiveReload() {
    ...
    if (
      ...
      !window.location.href.match(/(2016|ngrok)/)
    ) {
      return false
    }
  ...
  getWidgetDefs(el) {
    ...
    let widgets = (el.attr('data-widget') || el.attr('data-widgets')).split(
      /\,\s*/g
    )
          ...
          if (data.widgetOptional === true) {
            ...
            console.error(
              "Could not initialize widget '" +
                widget.name +
                "', as no widget with this name has been declared."
            )
            return
          }
        }
  ...
  widget(name, def, explicitBase) {
    ...
    if (def.use) {
      if (Array.isArray(def.use)) {
        ...
        console.warn(
          `The ${name} widget has property 'use' but it is not an array.`
        )
      }
    }
    if (name.indexOf('.') === -1) {
  ...
  resizeToFit(width, height, viewportWidth, viewportHeight, cover) {
    ...
    if (
      (cover && width / height > viewportWidth / viewportHeight) ||
      (!cover && width / height < viewportWidth / viewportHeight)
    ) {
  ...
  getRefreshes(target) {
    const itemsToRefresh = target
      .find('[data-xhr-refresh]')
      .addBack('[data-xhr-refresh]')
    ...
    const items = itemsToRefresh.filter((el) => {
      ...
      if (!id) {
        console.warn(
          'Refreshed item',
          el,
          'is missing an id attribute. Include one to refresh the element on page change'
        )
        return false
      }
  ...
  doRefreshes(result) {
    const $page = this.XHRPageContainer.parent()
      .children()
    ...
    this.getRefreshes($page).forEach((item) => {
      ...
      const newRefresh = result
        .find(`#${id}[data-xhr-refresh]`)
        .addBack(`#${id}[data-xhr-refresh]`)
  ...
  goToURL(url, dontPush) {
    ...
    if (this.xhrOptions.scrollAnimation) {
      var htmlBody = $('html,body')
        .stop(true)
        .animate({ scrollTop: 0 }, this.xhrOptions.scrollAnimation)
        .one('scroll', () => {
          htmlBody.stop(true)
        })
    }
    ...
    this.getContent(originalURL, (response, textStatus) => {
      ...
      var foundPageContainer = result
        .find('.xhr-page-contents, [data-page-container]')
        .first()
      ...
      if (foundPageContainer.length === 0) {
        ...
        console.error(
          'Could not find an element with a `data-page-container` attribute within the fetched XHR page response. Sending user directly to the page.'
        )
        return
      }

      // Grab content
      var newContent = $("<div class='xhr-page-contents'></div>").append(
        foundPageContainer.children()
      )
      ...
      var finalize = () => {
        ...
        var includes = result
          .find('headfake')
          .find('script, link[rel=stylesheet]')

        // Grab the body class
        var bodyClass = result.find('bodyfake').attr('class')
        bodyClass = this.xhrOptions.filterBodyClasses(
        ...
        if (oldContent.length === 0) {
          this.XHRPageContainer.wrapInner(
            "<span class='xhr-page-contents'></span>"
          )
      ...
      if (this.xhrOptions.loadImages) {
        this.preloadContent(
          newContent,
          this.xhrOptions.imageLoadTimeout,
          finalize
        )
      } else {
        finalize()
          ...
          if (widget.__promiseToLoad) {
            widget.__promiseToLoad.then(() => {
              widget._transitionIn(
                newState,
                oldState,
                this.xhrOptions.widgetTransitionDelay
              )
            })
          } else {
            var delay = widget._transitionIn(
              newState,
              oldState,
              this.xhrOptions.widgetTransitionDelay
            )
  ...
  transitionWidgetsOut(targetEl, newState, oldState, destroy, callback) {
    ...
    for (let widget of widgets) {
      ...
      if (widget._transitionOut) {
        var delay = widget._transitionOut(
          newState,
          oldState,
          this.xhrOptions.widgetTransitionDelay
        )
  ...
  handleXHRLinks(targetEl) {
    ...
    targetEl.find('a').each((index, el) => {
      ...
      if (
        linkEl.parents('#wpadminbar').length ||
        linkEl.parents('[data-prevent-xhr]').length
      )
        return
      ...
      if (this.xhrOptions.cachePages) {
        if (
          !linkEl.data('no-preload') &&
          linkEl.parents('[data-no-preload]').length === 0
        ) {
